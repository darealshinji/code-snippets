/**
 * MSVC-compatible implementation of basename() and dirname(),
 * taken from mingw-w64 v5.0.3.
 *
...
/* no VLAs with MSVC and somehow allocation issues */
#define ASSUMED_MAX_PATH 4096
...
#ifdef _MSC_VER
# ifdef __cplusplus
    wchar_t *refcopy = new wchar_t[1 + len];
# else
    wchar_t refcopy[ASSUMED_MAX_PATH];
# endif
#else
    wchar_t refcopy[1 + len];
#endif
...
# endif
     ...
     * step over the drive designator, if present ...  */
      ...
      ((refpath[0] >= L'A' && refpath[0] <= L'Z') ||
       (refpath[0] >= L'a' && refpath[0] <= L'z')))
    {
      refpath += 2;
      has_drive_designator = 1;
    }
    /* check again, just to ensure we still have a non-empty path name ... */
    if (*refpath)
    {
...
{
  char *orig, *copy1, *copy2;
  orig = "C:\\DirA\\\\DirB\\file.ext";
  copy1 = strdup(orig);
  copy2 = strdup(orig);
  printf("\noriginal:\n%s\n\n", orig);
  printf("basename:\n%s\n\n", basename(copy1));
